...
class _AchadosPageState extends State<AchadosPage> {
  ...
  // State
  List<ItemDTO> _itensAchados = [];
  List<ItemDTO> _itensAchadosFiltrados = [];
  ...
  Widget _buildItemsList() {
    if (_itensAchadosFiltrados.isEmpty) {
      return _buildEmptyState();
    }

    return ListView.builder(
      padding: const EdgeInsets.only(bottom: 16, left: 3, right: 3),
      itemCount: _itensAchadosFiltrados.length,
      itemBuilder: (context, index) {
        final item = _itensAchadosFiltrados[index];
