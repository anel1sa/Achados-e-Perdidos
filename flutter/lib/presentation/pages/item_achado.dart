...
class _ItemAchadoPageState extends State<ItemAchadoPage> {
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: widget.item.fotos?.isNotEmpty == true
                  ? ClipRRect(
                      borderRadius: BorderRadius.circular(12),
                      child: _buildItemImage(widget.item.fotos![0].url),
                    )
                  : _buildDefaultImage(),
            ),
            ...
            // Descrição
            _buildDetailField('Descrição:', widget.item.descricao ?? 'Sem descrição'),
            const SizedBox(height: 16),

            // Local encontrado
            _buildDetailField('Encontrado em:', widget.item.descLocalItem),
            const SizedBox(height: 16),

            // Horário (removido - campo não existe mais no novo schema)
            _buildDetailField(
              'Data de cadastro:',
              widget.item.dtaCriacao != null
                  ? '${widget.item.dtaCriacao!.day}/${widget.item.dtaCriacao!.month}/${widget.item.dtaCriacao!.year} ${widget.item.dtaCriacao!.hour}:${widget.item.dtaCriacao!.minute.toString().padLeft(2, '0')}'
                  : 'Não informado',
            ),
  ...
  void _confirmarItem() async {
    // Verificar se temos o ID do usuário relator
    if (widget.item.usuarioRelatorId == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Não foi possível abrir o chat. Item sem usuário relator.'),
    ...
    // Pegar o userId do storage
    final secureStorage = const FlutterSecureStorage();
    final currentUserId = await secureStorage.read(key: StorageKeys.userId);
    
    if (currentUserId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Você precisa estar logado para enviar mensagens'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    String nomeUsuario = widget.item.usuarioRelatorNome ?? 'Usuário';
    if (nomeUsuario == 'Usuário' || nomeUsuario.isEmpty) {
      try {
        final usuarioService = UsuarioService();
        final usuario = await usuarioService.getUsuarioById(widget.item.usuarioRelatorId);
        nomeUsuario = usuario.nome;
      } catch (e) {
        print('Erro ao buscar nome do usuário: $e');
        // Continua com 'Usuário' como fallback
      }
    }

    // Criar um objeto ConversaInfo para representar quem postou o item
    final conversaInfo = ConversaInfo(
      userId: widget.item.usuarioRelatorId.toString(),
      nome: nomeUsuario,
      iniciais: _getIniciais(nomeUsuario),
      ultimaMensagem: '',
      horario: '',
    );
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => IndividualChatPage(
          ...
          mensagemInicial: 'Olá, o ${widget.item.nome} é meu',
        ),
      ),
    );
