...
PS> aws ecr create-repository --repository-name achados-e-perdidos-api --region sa-east-1
...
PS> aws apprunner start-deployment --service-arn <ARN_DO_SERVIÇO> --region sa-east-1

Para recuperar o ARN do serviço:

PS> aws apprunner list-services --region sa-east-1 --query "ServiceSummaryList[?starts_with(ServiceName, 'achados')].{Name:ServiceName,ARN:ServiceArn}"
